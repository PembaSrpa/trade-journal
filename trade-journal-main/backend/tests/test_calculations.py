...
def test_calc_pips_long_win():
    assert calc_pips("EUR/USD", "long", 1.0842, 1.0891) == 49.0


def test_calc_pips_long_loss():
    assert calc_pips("EUR/USD", "long", 1.0842, 1.0812) == -30.0


def test_calc_pips_short_win():
    assert calc_pips("EUR/USD", "short", 1.0842, 1.0812) == 30.0


def test_calc_pips_short_loss():
    assert calc_pips("EUR/USD", "short", 1.0842, 1.0891) == -49.0


def test_calc_pips_jpy_pair():
    assert calc_pips("USD/JPY", "long", 150.00, 150.50) == 50.0


def test_calc_pnl_standard_lot():
    pnl = calc_pnl("EUR/USD", "long", 1.0842, 1.0891, 1.0, "standard", 0, 0)
    assert pnl == 490.0


def test_calc_pnl_mini_lot():
    pnl = calc_pnl("EUR/USD", "long", 1.0842, 1.0891, 1.0, "mini", 0, 0)
    assert pnl == 49.0


def test_calc_pnl_micro_lot():
    pnl = calc_pnl("EUR/USD", "long", 1.0842, 1.0891, 1.0, "micro", 0, 0)
    assert pnl == 4.9


def test_calc_pnl_subtracts_commission_adds_swap():
    pnl = calc_pnl("EUR/USD", "long", 1.0842, 1.0891, 1.0, "standard", 5, 2)
    assert pnl == 487.0


def test_calc_pnl_loss():
    pnl = calc_pnl("EUR/USD", "long", 1.0891, 1.0842, 0.1, "standard", 0, 0)
    assert pnl < 0
